...
function Background() {
  ...
  const handlePrevious = () => {
    imgIndex === 0
      ? setImgInex(backGround.length - 1)
      : setImgInex(imgIndex - 1);
  };
  const handleNext = () => {
    imgIndex === backGround.length - 1
      ? setImgInex(0)
      : setImgInex(imgIndex + 1);
  };
  return (
      ...
      >
        ...
        >
          <div className={cx("contain-info", "position-relative")}>
            ...
            <span className={cx("d-block", "fs-3", "text-center")}>
              Chất lượng tạo nên thương hiệu
            </span>
            <Link to={""} className={cx("btn-href")}>
              XEM NGAY
            </Link>
          </div>
        </div>
