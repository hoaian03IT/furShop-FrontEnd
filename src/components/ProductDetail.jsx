...
export const ProductDetail = ({ product }) => {
    ...
    const { _id, productName, brand, price = 0, discount = 0, attributes, shop } = product;
    ...
    return (
        <Row md={{ cols: 2 }} className={cx("details", "g-5")}>
            <Col>
                <Link
                    to={pathname.shop.split(":")[0] + shop?.tagname}
                    className="text-black mb-2 d-flex align-items-center">
                    <CiShop className="fs-1" />
                    <span className="ps-2 fs-5 fw-semibold text-decoration-underline">{shop?.name}</span>
                </Link>
            ...
            <Col>
                <div className={cx("content")}>
                    ...
                    <div className={cx("prices", "mt-4")}>
                        ...
                        <span className={cx("save-money")}>{`(Tiết kiệm ${formatCurrencyVND(price * discount)})`}</span>
                    </div>
                    ...
                    <div className={cx("color")}>
                        ...
                        <div className={cx("colors")}>
                            {colors.map((color) => (
                                <button
                                    key={color}
                                    className={cx("color-btn", color === selectedAttributes.color ? "active" : "")}
                        ...
                        <div className={cx("sizes")}>
                            {selectedAttributes.color ? (
                                sizes.map((size) => (
                                    <button
                                        key={size}
                                        className={cx("size-btn", selectedAttributes.size === size ? "active" : "")}
